...
class StorageRepositoryImplementation
    implements IStorageRepositoryImplementation {
  ...
  @override
  Future<Either<Failure, bool>> deleteStorageItem({required int id}) async {
    try {
  ...
  Future<Either<Failure, int>> updateStorageItem(
    ...
    try {
      final localStorageItem =
          await localDataSource.updateStorageItem(storageItem);
      return Right(localStorageItem);
    } on CacheException {
      return Left(CacheFailure(errorMessage: 'No local data found'));
    }
